...
void Character::Update(const SceneContext& sceneContext)
{
	if (m_pCameraComponent->IsActive())
	{
		...
		if (InputManager::IsMouseButton(InputState::down, leftMouseButton))
		{
			const POINT& lookValue{ InputManager::GetMouseMovement() };
			look.x = static_cast<float>(lookValue.x);
			look.y = static_cast<float>(lookValue.y);
		}

		//Optional: in case look.x AND look.y are near zero, you could use the Right ThumbStickPosition for look
		if (abs(look.x) < epsilon && abs(look.x) < epsilon)
		{
			const auto& lookValue{ sceneContext.pInput->GetThumbstickPosition(false) };
			look.x = lookValue.x;
			look.y = -lookValue.y;
		}
		...
		const float rotation{ elapsedSec * m_CharacterDesc.rotationSpeed };

		//Rotate this character based on the TotalPitch (X) and TotalYaw (Y)
		m_TotalYaw += look.x * rotation;
		m_TotalPitch += look.y * rotation;
