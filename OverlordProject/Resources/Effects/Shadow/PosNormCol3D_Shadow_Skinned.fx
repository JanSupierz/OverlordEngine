...
VS_OUTPUT VS(VS_INPUT input){
	...
	skinTransform += gBones[int(input.BoneIndices.x)] * input.BoneWeights.x;
	skinTransform += gBones[int(input.BoneIndices.y)] * input.BoneWeights.y;
	skinTransform += gBones[int(input.BoneIndices.z)] * input.BoneWeights.z;
	skinTransform += gBones[int(input.BoneIndices.w)] * input.BoneWeights.w;

	float4 finalPosition = mul(float4(input.pos, 1.f), skinTransform);

	float4x4 skinTransformTranspose = transpose(skinTransform);
	float3 finalNormal = mul(input.normal, (float3x3)skinTransformTranspose);
