...
{
	float3 totalSpecular = (0.f,0.f,0.f);
	
	//Combine the values
	if (gUseSpecularBlinn)
	{
		totalSpecular += CalculateSpecularBlinn(viewDirection, normal, texCoord);
	}
	
	if (gUseSpecularPhong)
	{
		totalSpecular += CalculateSpecularPhong(viewDirection, normal, texCoord);
	}
	
	//Multiply with the sampled value
	if (gUseTextureSpecularIntensity)
	{
		totalSpecular *= gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).r;
	}

	return saturate(totalSpecular);
}
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	{	
		...
		//Transform the normal map from [0,1] to [-1,1] range
		...
		newNormal = normalize(mul(float4(normalMap, 0.0f), tangentSpaceAxis).xyz);
	}	
...
{
	float3 environmentColor = (0.0f, 0.0f, 0.0f);
