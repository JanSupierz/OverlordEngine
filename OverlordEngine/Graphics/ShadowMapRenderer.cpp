...
void ShadowMapRenderer::Initialize()
{
	...
	//- The effect has two techniques, one for static meshes, and another for skinned meshes 
	
	m_pShadowMapGenerator = MaterialManager::Get()->CreateMaterial<ShadowMapMaterial>();

	//- We want to store the TechniqueContext for both techniques in the m_GeneratorTechniqueContexts array.
	//- Use the ShadowGeneratorType enum to retrieve the correct TechniqueContext by ID, 
	//	and also use that ID to store it inside the array
	constexpr int staticMeshes{ static_cast<int>(ShadowGeneratorType::Static) };
	constexpr int skinnedMeshes{ static_cast<int>(ShadowGeneratorType::Skinned) };

	m_GeneratorTechniqueContexts[staticMeshes] = m_pShadowMapGenerator->GetTechniqueContext(staticMeshes);
	m_GeneratorTechniqueContexts[skinnedMeshes] = m_pShadowMapGenerator->GetTechniqueContext(skinnedMeshes);
}
...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	const int type{ static_cast<int>(pMeshFilter->HasAnimations() ? ShadowGeneratorType::Skinned : ShadowGeneratorType::Static) };

	//2. Retrieve the corresponding TechniqueContext from m_GeneratorTechniqueContexts array (Static/Skinned)
	const MaterialTechniqueContext& technique{ m_GeneratorTechniqueContexts[type] };
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	const float viewWidth{ 400.f * sceneContext.aspectRatio };
	constexpr float viewHeight{ 400.f };
	...
	const XMVECTOR eyePosition{ XMLoadFloat4(&light.position) };

	//focusPosition: Calculate using the Direction Light position and direction
	const XMVECTOR focusPosition{ eyePosition + XMLoadFloat4(&light.direction) };

	//- Use the Projection & View Matrix to calculate the ViewProjection of this Light, store in m_LightVP
	const XMMATRIX viewMatrix{ XMMatrixLookAtLH(eyePosition, focusPosition, XMVectorSet(0, 1, 0, 0)) };
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	for (const auto& mesh : pMeshFilter->GetMeshes())
	{
		//- Set VertexBuffer
		const UINT offset{ 0 };
		const auto& vertexBufferData{ pMeshFilter->GetVertexBufferData(technique.inputLayoutID, mesh.id) };
		...
		m_pShadowMapGenerator->SetTechnique(type);
		...
		for (uint32_t index{}; index < techDesc.Passes; ++index)
		{
			technique.pTechnique->GetPassByIndex(index)->Apply(0, pDeviceContext);
